...
\textbf{Update --- HuggingFace re-validation on NAVI.}
After the seven primary experiments, we re-implemented the LoRA + HardInfoNCE + Safe-Radius recipe on top of the HuggingFace \texttt{Dinov3ViTModel} (S/16 and L/16 variants) and re-evaluated on NAVI under a 1024$\times$1024 evaluation coordinate system with \texttt{batch\_size}\,$=$\,8 (true cross-image negatives). Under this setting the LoRA-tuned models did exceed the zero-shot Precision baseline (Section~\ref{sec:phase7}). Crucially, however, our diagnostic suite (Section~\ref{sec:phase8}) shows that this gain is \emph{not} the model learning new geometric features: the positive-pair cosine distribution barely moves (Zero-Shot $\mu=0.896$ vs.\ LoRA $\mu=0.900$), the per-pair correlation between matching gain and pose gain is near zero (Pearson $0.171$, with $317{\,/\,}908$ pairs improving Precision while \emph{worsening} pose), and the effective rank actually drops slightly. The numerical improvement is therefore consistent with --- not a refutation of --- the Layer-4 (non-differentiable gap) and Layer-6 (information-theoretic ceiling) findings.
...
\subsection{Phase 7: HF Re-validation on NAVI --- A Numerical Surprise}
\label{sec:phase7}

\textbf{Motivation.} All Phase 1--6 experiments used the original DINOv3 \texttt{.pth} backbone with intra-image negatives (\texttt{batch\_size}\,$=$\,1). To rule out implementation-specific artefacts, we re-ran the strongest recipe (LoRA + HardInfoNCE + Safe Radius) on top of the HuggingFace \texttt{Dinov3ViTModel}, with \texttt{batch\_size}\,$=$\,8 (genuine cross-image negatives) and a fully held-out NAVI test split (908 pairs). Two backbone sizes are reported: \textbf{ViT-S/16} ($\sim$22M, ``small'') and \textbf{ViT-L/16} ($\sim$300M, ``middle''). Evaluation coordinate system is $1024\times1024$ to match the NAVI native max-dim, and Precision uses the same $5\times10^{-4}$ epipolar threshold as before; absolute Zero-Shot numbers therefore differ from Phase 0 (which used $640\times480$ ScanNet-style geometry).

\textbf{Method.} LoRA rank $r=4$, $\alpha=8$, applied to Q/V projections of every transformer block; HardInfoNCE with Safe-Radius\,$=$\,5 patches; AdamW lr$=$1e-4 with cosine schedule; 15 epochs; checkpoint every 3 epochs.

\textbf{Results.} Table~\ref{tab:phase7} summarises final-epoch metrics; Figure~\ref{fig:phase7_curve} plots Precision/AUC@10/AUC@20 for every saved epoch against the zero-shot baseline (dashed lines).

\begin{table}[H]
\centering\small
\caption{Phase 7: HF re-validation on NAVI test (908 pairs, $1024^2$ eval). LoRA improves over zero-shot in every metric for both backbones.}
\label{tab:phase7}
...
\caption{Phase 7 per-epoch metrics on NAVI for the two backbone sizes. Solid curves = LoRA-tuned; dashed lines = zero-shot baseline. Both backbones cross above the zero-shot Precision baseline by epoch 2--3 and stabilise around $+7$ percentage points.}
\label{fig:phase7_curve}
\end{figure}

\textbf{Reading the result.}
Taken at face value, Phase 7 contradicts Phases 2--4: the same family of methods that previously collapsed now reproducibly \emph{exceeds} zero-shot on every metric. Three engineering differences explain the gap:
\begin{enumerate}
    \item \textbf{True cross-image negatives.} \texttt{batch\_size}\,$=$\,8 means InfoNCE's $K{=}128$ negatives are sampled from \emph{seven other scenes} per anchor. The single-image-vs-many-images distinction was identified in Phase 3 as the root cause of the $\ln(K{+}1)$ collapse, and Phase 7 confirms it: the loss never approaches the $\ln(129)$ floor.
    \item \textbf{Safe Radius retained.} The 5-patch spatial mask remains in place, preventing the model from being asked to push neighbouring patches apart.
    \item \textbf{Different evaluation geometry.} Phase 0 used $640\times480$ on ScanNet; Phase 7 uses $1024\times1024$ on NAVI. Zero-shot Precision drops from 49\% / 32\% to 17\% under the new geometry, so the relative head-room for fine-tuning to claim a win is much larger.
\end{enumerate}

In short, Phase 7 is the recipe of Phase 3 with the two engineering bottlenecks of Phase 2 lifted; it does what Phase 3 was designed to do. The natural next question is whether the $+7\%$ Precision gain is genuinely \emph{geometric} learning, or merely a re-weighting of an already-existing semantic signal. This is what Phase 8 sets out to test.

\subsection{Phase 8: Diagnostic Suite --- The Improvement Is Cosmetic}
\label{sec:phase8}

We ran four feature-space diagnostics on the Phase-7 small / middle checkpoints. Together they argue that the Phase-7 numerical gain does \emph{not} reflect new geometric content in the features --- it is consistent with the Layer-4 and Layer-6 findings of the original report.

\paragraph{Diagnostic 1 --- Positive-pair cosine distribution (Layer 3).}
For each image we draw two random crops and pair patches that re-project to the same 3D point; their cosine similarity should rise if LoRA truly ``pulls positives together''. Figure~\ref{fig:hist_pos} plots the distribution. Zero-Shot and LoRA are essentially indistinguishable: $\mu_{ZS}=0.896$ vs.\ $\mu_{LoRA}=0.900$ for the middle backbone (and $0.947$ vs.\ $0.949$ for small). \emph{The contrastive loss is barely able to displace the positives at all} --- the geometric capacity of the feature space is already saturated.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{presentation/result/diag_middle/hist_pos_cos_middle.png}
\caption{Phase 8 / Layer-3 indicator (ViT-L/16). Positive-pair cosine distribution before vs.\ after LoRA fine-tuning. The two histograms overlap almost perfectly, supporting the Layer-6 claim that the geometric signal LoRA is trying to amplify has already been compressed away during pre-training.}
\label{fig:hist_pos}
\end{figure}

\paragraph{Diagnostic 2 --- Layer-4 non-differentiable-gap scatter.}
For each of the 908 NAVI test pairs we compare the per-pair Precision change against the per-pair pose-error change. If matching quality reliably translates into pose quality, points should fall along a positively-sloped line. Figure~\ref{fig:layer4} shows otherwise.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{presentation/result/diag_middle/layer4_scatter_middle.png}
\caption{Phase 8 / Layer-4 indicator (ViT-L/16). $x$: per-pair Precision change (LoRA $-$ Zero-Shot). $y$: per-pair pose-error change (Zero-Shot $-$ LoRA, i.e.\ positive\,$=$\,improved). Pearson $0.171$, Spearman $0.192$. Of 908 pairs, 317 (35\%) gain Precision but \emph{lose} pose accuracy. This is direct empirical evidence for the three non-differentiable gaps argued in Section~\ref{sec:layer4-original}.}
\label{fig:layer4}
\end{figure}

\paragraph{Diagnostic 3 --- Manifold-structure metrics (Layer 2).}
We further measure intra-image pairwise patch cosine, effective rank (entropy of the PCA spectrum, normalised by $\log D$), and ``neighbour dominance'' (fraction of patches whose top-1 NN is a 4-neighbour). Table~\ref{tab:diag-metrics} aggregates 30 NAVI test images.

\begin{table}[H]
\centering\small
\caption{Phase 8: feature-space metrics, Zero-Shot vs.\ LoRA (30 images). Differences are sub-percent for both backbones; the manifold itself is essentially unchanged.}
\label{tab:diag-metrics}
\begin{tabular}{@{}lcccc@{}}
\toprule
 & \multicolumn{2}{c}{\textbf{ViT-S/16}} & \multicolumn{2}{c}{\textbf{ViT-L/16}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
\textbf{Metric} & ZS & LoRA & ZS & LoRA \\
\midrule
mean intra-image cos        & 0.519 & 0.530 & 0.334 & 0.351 \\
mean effective rank         & 0.406 & 0.405 & 0.444 & 0.442 \\
mean positive-pair cos      & 0.947 & 0.949 & 0.896 & 0.900 \\
mean neighbour dominance    & 0.989 & 0.989 & 0.984 & 0.984 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Diagnostic 4 --- PCA visualisation.}
Figure~\ref{fig:pca-compare} compares the first three PCA components (rendered as RGB) of patch tokens, before and after LoRA, on the same image with a shared PCA basis. The two maps are visually near-identical --- consistent with the small numerical changes in Table~\ref{tab:diag-metrics}.

\begin{figure}[H]
\centering
\begin{subfigure}{0.95\textwidth}
\centering
\includegraphics[width=\textwidth]{presentation/result/diag_small/pca_small_004_compare.png}
\caption{ViT-S/16}
\end{subfigure}\\[3pt]
\begin{subfigure}{0.95\textwidth}
\centering
\includegraphics[width=\textwidth]{presentation/result/diag_middle/pca_middle_004_compare.png}
\caption{ViT-L/16}
\end{subfigure}
\caption{Phase 8 / Layer-2 indicator. PCA-RGB rendering of patch features (shared basis between Zero-Shot and LoRA). The structures are visually indistinguishable, consistent with Diagnostic 3 (Table~\ref{tab:diag-metrics}).}
\label{fig:pca-compare}
\end{figure}

\paragraph{Synthesis.}
The four diagnostics paint a coherent picture: under the Phase-7 setup, LoRA does not move the geometry of DINOv3's feature space in any measurable way (positive-pair cosine $\Delta < 0.005$; effective rank $\Delta \approx -0.002$; PCA visually identical). The Precision gain is therefore best read as a \emph{matching-decision-boundary re-weighting} that exploits residual geometric noise the foundation model already exposes --- not as the model acquiring new geometric distinctiveness. Importantly, this re-weighting is essentially uncorrelated with pose recovery (Pearson $0.171$), confirming the Layer-4 prediction that ``better matches'' need not translate into ``better poses''. Phase 7 thus becomes the \emph{strongest} confirmation of the original report's central thesis: the ceiling is set by what was preserved during pre-training, not by what the loss can teach during fine-tuning.
...
\midrule
\multicolumn{7}{l}{\textit{HF re-validation (NAVI test, 908 pairs, $1024^2$ eval; see Section~\ref{sec:phase7})}} \\
\multirow{2}{*}{7s} & \multirow{2}{*}{\color{green!55!black}LoRA-S + Safe Radius (HF)}
    & ZS-NAVI & 0.076 & 0.155 & 0.436 & 17.38\% \\
  & & FT-NAVI & 0.000 & 0.126 & 1.040 & \color{green!55!black}\textbf{25.06\%} \\
\midrule
\multirow{2}{*}{7m} & \multirow{2}{*}{\color{green!55!black}LoRA-L + Safe Radius (HF)}
    & ZS-NAVI & 0.059 & 0.172 & 0.456 & 16.75\% \\
  & & FT-NAVI & 0.061 & 0.184 & 1.123 & \color{green!55!black}\textbf{23.71\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\end{equation}
The model never sees how improving cosine similarity affects the final pose. ``Better features'' from the training perspective can easily produce ``worse poses'' from the evaluation perspective. Phase 8 (Section~\ref{sec:phase8}, Figure~\ref{fig:layer4}) provides direct empirical confirmation: per-pair Precision and pose-error gains are only weakly correlated (Pearson $0.171$), and 35\% of the test pairs improve in matching while \emph{regressing} in pose.
